...
/**
 * GET /api/simulations?rut=123
 * Lista las simulaciones del usuario (orden reciente primero)
 */
 ...
 * body: { rut: number, data: {...} }
 * Crea una simulación (el trigger limita a 5 por usuario)
 */
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (String(e.message || '').includes('SIM_LIMIT_REACHED')) {
      return res.status(409).json({ error: 'limit' }); // tope alcanzado
    }
 ...
 * Elimina una simulación del usuario
 */
...
/**
 * DELETE /api/simulations?rut=123
