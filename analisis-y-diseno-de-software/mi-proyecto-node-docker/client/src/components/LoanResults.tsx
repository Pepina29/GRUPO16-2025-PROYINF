...
const SIM_LIMIT = 5; // tope por usuario (coincide con la BD)
...
export const LoanResults = ({
  ...
  const handleGuardarPerfil = async () => {
    ...
    if (!current?.rut) {
      showToast("Inicia sesión", "Debes iniciar sesión para guardar simulaciones.", "error");
      return;
    }

    const sim = { monto, tasa: tasaInteres, cuotas, fechaPrimerPago, pagoPorCuota, montoTotal };
    ...
    try {
      ...
      if (res.status === 409) {
        showToast("Límite alcanzado", `Ya tienes ${SIM_LIMIT} simulaciones. Borra alguna en tu Perfil.`, "error");
        return;
      }
      ...
      // Notificar a Header/Perfil para refrescar conteos y lista
      window.dispatchEvent(new CustomEvent("simulations:changed"));
      showToast("Simulación guardada", "La encontrarás en tu Perfil.", "success");
    } catch (e: any) {
      showToast("No se pudo guardar", e?.message || "Inténtalo nuevamente.", "error");
    }
  };
  ...
  const handleSolicitar = () => {
    ...
    navigate("/solicitar-prestamo", {
      state: { monto, tasaInteres, cuotas, fechaPrimerPago, pagoPorCuota, montoTotal }
    });
  };

  const formatCurrency = (value: number) =>
    new Intl.NumberFormat("es-UY", { style: "currency", currency: "UYU", minimumFractionDigits: 0 }).format(value);
